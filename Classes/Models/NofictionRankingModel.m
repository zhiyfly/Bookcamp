...
#import "TFHpple.h"
#import "BookObject.h"
#define DoubanLatestBookPath @"//ul[@class='chart-dashed-list']//li[@class='clearfix']//p[@class='clearfix cover']//a" //div[@id='glide1']//div[@class='detail-frame']//h2 | 
@implementation NofictionRankingModel
...
- (void)load:(TTURLRequestCachePolicy)cachePolicy more:(BOOL)more {
	if (!self.isLoading ) {
		NSString* url =  [BookServerDistination(chart) stringByAppendingString:@"?subcat=I"];
    ...
    if ([keyPath isEqual:@"bookStatus"]) {
		...
		if ( bookStatus == Finish || bookStatus == Fail  ){
			...
			if (!numLatestBook) {
				...
				[ModelLocator sharedInstance].rankingNonfictionBooks = self.books;
			}
		}
    }
