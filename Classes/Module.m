...
-(void)setTabBarHidden:(BOOL)hidden{
	...
	if (hidden) {
		self.view.frame = TTRectContract(self.view.frame, 0, TabBarHeight);
	}
	else {
		self.view.frame = TTRectContract(self.view.frame, 0, -TabBarHeight);
	}
...
-(ModuleView*)view{
	if (!_view) {
		_view = [[ModuleView alloc] initWithFrame: TTRectContract(BKScreenBoundsWithoutBar(), 0, _tabBarHidden ?  0: TabBarHeight) ];
